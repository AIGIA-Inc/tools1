...
def index(request: Request, key: str = "", root: str = "admin@aigia.co.jp", layout: str = "dot"):
	if key is not None:
		...
		if key == _key:
			return templates.TemplateResponse("index.html", context={"request": request, "host": host, "path": path, "key": _key, "rootuser": root, "layout": layout})
		else:
...
def accounts(request: Request, key: str = "", root: str = "admin@aigia.co.jp"):
	if key is not None:
		...
		if _key == key:
			try:
				return templates.TemplateResponse("accounts.html", context={"request": request, "host": host, "path": path, "key": key, "rootuser": root})
...
def totalling(request: Request, key: str = "", root: str = "admin@aigia.co.jp", studio: str = ""):
	if key is not None:
		...
		if _key == key:
			try:
				return templates.TemplateResponse("totalling.html", context={"request": request, "host": host, "path": path, "key": key, "rootuser": root, "studio": studio})
			except Exception as e:
...
def guest(request: Request, key: str = "", root: str = "admin@aigia.co.jp"):
	if key is not None:
		host, path, _key, username, password = config()
		if _key == key:
			return templates.TemplateResponse("guests.html", context={"request": request, "host": host, "path": path, "key": key, "rootuser": root})
		else:
			raise HTTPException(status_code=403, detail="invalid key.")
	else:
		raise HTTPException(status_code=403, detail="no key.")
...
@app.get('/shots')
def shots(request: Request, key: str = "", query: Any = {}, sort: str = "platform.description.score", skip: int = 0, limit: int = 20):
...
def download_accounts(key: str = ""):
	...
	if key is not None:
		...
		if _key == key:
			...
			return FileResponse(path=output, media_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', filename="accounts.xlsx")
		else:
			raise HTTPException(status_code=403, detail="invalid key.")
	else:
		raise HTTPException(status_code=403, detail="no key.")
...
if __name__ == "__main__":
	uvicorn.run("aapi:app", host="127.0.0.1", port=5000, log_level="info")
